...
function sendAnnounce(bot, message, args, guild) {
  ...
  } else if (guild.channels.find("id", "422913280500301825")) {
      sendChannel = guild.channels.find("id", "422913280500301825");
  }else if (guild.channels.find("id", "478076401971363840")) {
      sendChannel = guild.channels.find("id", "478076401971363840");
  }else if (guild.channels.find("id", "voting")) {
      sendChannel = guild.channels.find("id", "voting");
    }

  let announce = new discord.RichEmbed()
    .setColor("7289DA")
    .setAuthor(`${bot.user.username} Announcement`)
    .setDescription(`**Hello Liam here, creator of Sad Rose just wanting you to know, **\n${args.join(' ')}\n\n**This message was sent to *${guild.name}* on purpose from the creator.`)
    .setFooter(`An official announcement from Liam. (liami98#6525)`, bot.user.avatarURL)
    .setTimestamp();
